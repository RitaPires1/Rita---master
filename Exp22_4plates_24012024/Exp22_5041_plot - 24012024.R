setwd("C:/Users/sergi/Downloads/ITQB/Plate readings/Exp22_4plates_24012024")

library(tidyverse)
library(readxl)

Exp22 <- Exp22_4plates_24012024 <- read_excel("Exp22_4plates_24012024_5041_final_final.xlsx")
# View(Exp22) 
...
Exp22_v2 <- Exp22_v2 %>% mutate(TPEN_conc = recode (TPEN_conc,  "ctrTPENcells" = "blank",
                                                             "ctrTPEN" = "0 uM",
                                                             "10.8" = "54 uM",
                                                             "12" = "60 uM",
                                                             "14" = "70 uM",
...
Exp22_v2$TPEN_conc <- factor(Exp22_v2$TPEN_conc, levels = c("blank",
                                                            "0 uM",
                                                            "54 uM",
                                                            "60 uM",
                                                            "70 uM",
...
Exp22_v5 <- Exp22_v4 %>% group_by(Time_h, Strain, Transfer_medium, TPEN_conc) %>%
  summarise(ODmean = mean(OD), sdOD = sd(OD), n())


Exp22_v5 <- Exp22_v5 %>% mutate(TPEN_conc = recode (TPEN_conc,
                                                    "ctrTPENcells" = "blank",
                                                    "ctrTPEN" = "0 uM",
                                                    "10.8" = "54 uM",
                                                    "12" = " 60 uM",
...
Exp22_v5  %>% filter (Transfer_medium == "mGAM") %>%
  ggplot(aes(x = Time_h,
             y = ODmean,
