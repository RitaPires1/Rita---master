...
#It's better to filter the data --> better results
filtered_data <- Exp38_v2 %>%
  filter(!(Community == "C2" & Tech_Repl == "TechRep2" & TPEN_or_ZnCl2_conc == "15uM"))
...
Exp38_v3<- filtered_data %>% group_by(Community, Transfer_medium, Zn_condition, Tech_Repl, Biol_Repl, Well_number) %>%
...
#Final plot taking into account filtered data used in the 1st plot
Exp38_v5  %>%
  ggplot(aes(x = Time_h,
            y = ODmean,
