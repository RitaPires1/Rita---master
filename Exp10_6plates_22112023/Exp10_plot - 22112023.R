setwd("~/ITQB/Plate readings/Exp10_6plates_22112023")
...
Exp10_v2 %>% filter (Transfer_medium == "M8") %>%
  ggplot(aes(x= Time_h, y=OD_578, color = Tech_Repl)) +
  geom_line() +
  facet_grid (Strain ~ TPEN_conc)
...
# Filter the data for the first 24 hours - M8
Exp10_v24h <- Exp10_v5 %>% filter(Time_h <= 24)
                            

#Filter strains which did not use - M8
Exp10_filtered <- Exp10_v24h %>%  
  filter(Strain != "14115") 

# Plot the data filtered for the first 24 hours
Exp10_filtered %>% 
  filter(Transfer_medium == "M8") %>%
  ggplot(aes(x = Time_h, y = ODmean, color = Transfer_medium)) +
  geom_point() +
  geom_line() +
  ...
  facet_grid(Strain ~ TPEN_conc) +
  theme_bw() +
  theme(axis.text = element_text(size = 6))  # Adjust axis text size if needed

#Filter strains which did not use
Exp10_v5_filtered <- Exp10_v5 %>%  
  filter(Strain != "14115")

Exp10_v5_filtered  %>% filter (Transfer_medium == "mGAM") %>%
  ggplot(aes(x = Time_h,
             y = ODmean,
